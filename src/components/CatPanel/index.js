...
export default class CatPanel extends Component {
  ...
  render() {
    ...
    return (
      <View className={`cat-panel ${className} ${noBackgroundCls} ${noBorderCls} ${noPaddingCls}`}>
        <View className={"cat-panel_header"}>
          {
            title &&
            <View className={`cat-panel-title ${titleClass}`}>{title}</View>
          }
          {
            isLink &&
            <View className='cat-panel-link' onClick={this.handleClickLink}>
              {
                linkDesc &&
                <View className="cat-panel-link-desc">{linkDesc}</View>
              }
